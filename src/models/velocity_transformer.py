...
class VelocityNetwork(nn.Module):
    """Neural network for predicting the velocity field for both R3 and SO3 using transformers."""
    ...
    def __init__(self, config: BaseExperimentConfig):
        ...
        self.config = config
        hidden_dim: int = 128  # Increased for transformer capacity
        num_heads: int = 8
        num_layers: int = 4
        dropout: float = 0.1
        activation = nn.GELU
        ...
        # Separate input projections for R3 and SO3
        self.r3_input_proj = nn.Linear(3, hidden_dim)
        self.so3_input_proj = nn.Linear(9, hidden_dim)  # Will flatten 3x3 to 9
        ...
        # Separate output heads for R3 and SO3
        self.r3_head = nn.Linear(hidden_dim, 3)
        self.so3_head = nn.Linear(hidden_dim, 9)  # Will reshape back to 3x3

        # Initialize output heads with small weights for stability
        nn.init.zeros_(self.r3_head.weight)
        nn.init.zeros_(self.r3_head.bias)
        nn.init.zeros_(self.so3_head.weight)
        nn.init.zeros_(self.so3_head.bias)

        self.scale = nn.Parameter(torch.ones(1) * 0.1)

    def forward(self, r3_x: Tensor, so3_x: Tensor, t: Tensor) -> Tuple[Tensor, Tensor]:
        """Forward pass through the velocity network.

        Args:
            r3_x: R3 input tensor of shape [batch_size, 3]
            so3_x: SO3 input tensor of shape [batch_size, 3, 3]
            t: Time tensor of shape [batch_size, 1]

        Returns:
            Tuple of (r3_velocity, so3_velocity):
                - r3_velocity: [batch_size, 3]
                - so3_velocity: [batch_size, 3, 3]
        """
        # Ensure inputs are properly shaped
        if r3_x.dim() == 1:
            r3_x = r3_x.unsqueeze(0)
        if so3_x.dim() == 2:
            so3_x = so3_x.unsqueeze(0)
        ...
        # Flatten SO3 input
        so3_flat = rearrange(so3_x, "b c d -> b (c d)", c=3, d=3)

        # Time embedding
        t_emb = self.time_proj(t)

        # Project inputs separately
        r3_h = self.r3_input_proj(r3_x)
        so3_h = self.so3_input_proj(so3_flat)

        # Stack features for transformer processing
        # [batch_size, 3, hidden_dim] where the 3 positions are:
        # [time_embedding, r3_features, so3_features]
        h = torch.stack([t_emb, r3_h, so3_h], dim=1)

        # Process through transformer
        # The transformer will allow each component to attend to the others
        h = self.transformer(h)

        # Extract relevant features (taking r3 and so3 positions)
        r3_features = h[:, 1]
        so3_features = h[:, 2]

        # Additional processing
        r3_h = self.post_transformer(r3_features)
        so3_h = self.post_transformer(so3_features)

        # Generate outputs through separate heads
        r3_velocity = self.r3_head(r3_h) * self.scale
        so3_velocity_flat = self.so3_head(so3_h) * self.scale

        # Reshape SO3 output back to 3x3
