...
def print_pose_comparison(
    intended_SE3: np.ndarray,
    actual_pos: List[float],
    actual_quat: List[float],
    threshold: float = 0.01,
):
    """Helper function to compare intended and actual poses"""
    intended_pos = intended_SE3[:3, 3]
    intended_rot = intended_SE3[:3, :3]
    intended_quat = rot2quat(intended_rot)

    pos_error = np.linalg.norm(np.array(intended_pos) - np.array(actual_pos))
    quat_error = np.linalg.norm(np.array(intended_quat) - np.array(actual_quat))

    print("\nPose Comparison:")
    print(f"Position error: {pos_error:.4f} meters")
    print(f"Quaternion error: {quat_error:.4f}")
    print(f"Within threshold: {pos_error < threshold and quat_error < threshold}")
    print(f"Intended position: {intended_pos}")
    print(f"Actual position: {actual_pos}")
    print(f"Intended quaternion: {intended_quat}")
...
class PandaRobot:
    ...
    def initialize_simulation(self, gravity: bool = False) -> None:
        ...
        if self.config.object_filepath:
            print(f"\nLoading test object from: {self.config.object_filepath}")
            if not os.path.exists(self.config.object_filepath):
                raise FileNotFoundError(
                    f"Test object mesh not found at: {self.config.object_filepath}"
                )
            ...
            try:
                ...
                print("Test object loaded successfully")
                print(f"Object mass: {self.config.object_mass}kg")
                print(f"Object friction: {self.config.object_friction}")
                print(f"Object COM: {self.config.object_com}")
                ...
                print(f"Error loading test object: {e}")
                raise
    ...
    def SE3_to_joint_positions(self, SE3: np.ndarray) -> List[float]:
        ...
        print("\nCurrent joint positions before IK:")
        for i, pos in enumerate(current_poses):
            print(f"Joint {i}: {pos:.4f} rad ({pos * 180 / np.pi:.1f} deg)")
    ...
    def move_to_pose(self, SE3: np.ndarray, duration: float = 2.0) -> None:
        ...
        try:
            ...
            steps = int(duration / self.config.stepsize)
            print(f"Interpolating over {steps} steps")
            ...
            for i in range(steps):
                ...
                if i % (steps // 5) == 0:  # Print progress every 20%
                    print(f"\nMove progress: {(i+1)/steps*100:.1f}%")
                    current_pos = log_joint_positions(self.robot, self.arm_joint_ids)

            # Verify final position
            final_pos, final_orn = p.getLinkState(self.robot, self.ee_id)[:2]
            print_pose_comparison(SE3, final_pos, final_orn)
